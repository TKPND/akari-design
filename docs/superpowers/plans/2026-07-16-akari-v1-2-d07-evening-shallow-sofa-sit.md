# Akari v1.2 Daily.2 D07 Evening Shallow Sofa Sit Implementation Plan

> Execute autonomously under the user's delegated A/B selection authority.

**Goal:** Build, select, promote, verify, and integrate D07.

**Architecture:** Extend the Daily registry and validator without changing
accepted D01-D06 or Core bytes. Generate local A/B from accepted D06 and four
Core controllers, then promote one byte-for-byte.

## Task 1: Register

1. Add failing tests for D07 asset, request, references, dependencies, counts,
...
3. Run the D07 gate, Markdown lint, and `git diff --check`; commit.

## Task 2: Generate and Select

1. Open D06, C04, C03, C06-2, and C07 seated immediately before each call.
...
2. Record ordered reviews, promote byte-for-byte, and set D07 accepted r01.
3. Run the D07 edit and v1.2 integration gates, commit durable files only,
   preserve local evidence in the primary checkout, fast-forward main,
   reverify, clean the worktree, and continue to D08.
