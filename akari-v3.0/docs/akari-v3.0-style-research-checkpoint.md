...
> 履歴資料：このメモは2026年8月の別デザインの研究記録です。
> 2026年9月7日以降の黒髪ボブ版v3.0は [現在の作業パッケージ](../README.md) を参照してください。
> このメモの画像や指定を、現在の生成参照へ混ぜないでください。

Date: 2026-08-19

Status: V2.3 face anchor user-promoted on 2026-08-25; reproducibility gate remained 5/6
...
作れていない。この判定時点では、1枚目、A群、B群、C群のいずれも
正式アンカーへ昇格しなかった。

その後2026-08-25に、ユーザーは元の`20260819_219844051.jpeg`を
V2.3としてcommitするよう明示した。これにより、同一SHA-256のJPEGを
`akari-v2.3/accepted/base/akari-v2.3-face-anchor.jpeg`へ無変換で昇格した。
この決定は1枚目を正式なV2.3顔アンカーにするが、A群、B群、C群、
立ち姿と座り姿のホールドアウトを正準画像へ昇格するものではない。

追加で再現性を高める場合は、Bを遠い目と片側の頬が縮む程度まで
斜め角度を強め、Cを顔の画素数を確保した全身構図で再試験する。
この追加試験はV2.3顔アンカー昇格の前提ではない。駅のホールドアウト試験は、
別の明示的な進行判断を受けるまで始めない。
...
元のV2.3顔候補だけは、2026-08-25のユーザー指示により正式資産へ昇格した。
